...
Jira dedupe/create/link work should use the main triage loop's same-round snapshot plus API/connector evidence. Use Chrome for Sentry/Jira visual mutations only when a current step explicitly needs UI verification or an API-safe path is not available.
