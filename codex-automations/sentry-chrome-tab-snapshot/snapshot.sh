...
sync_exit=0
if [[ -f "$SCRIPT_DIR/sync-live-report.mjs" && -f "$FULL_JSON" && "${SENTRY_CHROME_RUN_LEGACY_8766_SYNC:-0}" =~ ^(1|true|yes)$ ]]; then
  if ! "$NODE_BIN" "$SCRIPT_DIR/sync-live-report.mjs" "$FULL_JSON"; then
    sync_exit=1
    log "LIVE_REPORT_SYNC_WARNING: snapshot JSON was written and remains authoritative; optional localhost:8766 sync failed; see $SCRIPT_DIR/live-report-sync-status.json"
  fi
fi

if [[ "$snapshot_exit" -eq 0 && -f "$SCRIPT_DIR/scan-issue-jira-links.mjs" && "${SENTRY_CHROME_RUN_LEGACY_8766_DETAIL_SCAN:-0}" =~ ^(1|true|yes)$ ]]; then
  if ! "$NODE_BIN" "$SCRIPT_DIR/scan-issue-jira-links.mjs" "$FULL_JSON"; then
    log "ISSUE_JIRA_SCAN_WARNING: fresh snapshot succeeded, but optional legacy localhost:8766 detail scan had warnings; see $SCRIPT_DIR/issue-jira-scan-status.json"
  fi
fi
