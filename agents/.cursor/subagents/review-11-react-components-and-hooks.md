---
name: review-11-react-components-and-hooks
description: "Review **changed** TSX/hooks for React correctness, readability, and alignment with **project** rules (`react-standard`, `jsx-best-practices`, `functional-principles`, `zustand-memo-optimization` when relevant)."
model: inherit
readonly: true
---
...
- [ ] **Conditional JSX:** nullable/boolean rendering follows **`renderIfNotNull` / `renderWithCondition`** where the codebase does (see `code-standard.mdc`); no risky `&&` on numbers/strings if that violates local pattern.
