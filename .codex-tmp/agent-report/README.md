...
```bash
AGENT_REPORT_HOST=127.0.0.1 AGENT_REPORT_SEND_STATUS=0 /Users/thiennguyen/thiennp.github.io/agent-report/scripts/ensure-agent-report-server.sh
```
...
AGENT_REPORT_WS=ws://127.0.0.1:3100/stream npm run send -- --automation-name "<automation name>" --title "<item or step title>" --status <running|success|warning|blocked|error|pending|info> "<concise update>"
```
...
Use the **Forward reports** form on the page to add a target port, such as `3200`, or a full WebSocket URL, such as `ws://127.0.0.1:3200/stream`.
...
```bash
AGENT_REPORT_WS=ws://127.0.0.1:3100/stream npm run send -- "Running on a custom port"
```
